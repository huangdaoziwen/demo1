...
class Domain {
 @Entity @Table(name="users", uniqueConstraints=@UniqueConstraint(columnNames="email")) static class User {
  ...
  @ElementCollection Set<String> allergens=new HashSet<>(); @ElementCollection Set<String> preferredCategories=new HashSet<>();
  String spice="None"; Integer budgetMin=1500,budgetMax=4000;
 }
 @Entity static class Dish {
  ...
  String category,protein,spice,image; boolean active=true; @ElementCollection Set<String> allergens=new HashSet<>();
  @Column(length=2000) String optionsJson="[]";
 }
 @Entity @Table(uniqueConstraints=@UniqueConstraint(columnNames={"dish_id","menuDate"})) static class DailyMenu {
 ...
 @Entity @Table(name="meal_orders",uniqueConstraints=@UniqueConstraint(columnNames={"user_id","mealDate","slot","activeKey"})) static class MealOrder {
  ...
  @OneToMany(cascade=CascadeType.ALL,orphanRemoval=true,mappedBy="order") List<OrderItem> items=new ArrayList<>();
 }
 @Entity static class OrderItem {
